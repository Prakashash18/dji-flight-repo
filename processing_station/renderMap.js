...
var renderMap = async function(videoFile, flightLogFile) {
    ...
    // parse the flight log from CSV or SRT to an array of objects
    var observations;
    if (flightLogFile.name.toLowerCase().endsWith('.srt')) {
        observations = await parseSRTFile(flightLogFile);
    } else {
        observations = await readCSVFile(flightLogFile);
    }
...
const parseSRTFile = function(file) {
    return new Promise(function(resolve, reject) {
        ...
        reader.onload = function (e) {
            ...
            for (var i = 0; i < blocks.length; i++) {
                ...
                var timeMatch = blockLines[1].match(/(\d{2}):(\d{2}):(\d{2}),(\d{3})\s*-->\s*(\d{2}):(\d{2}):(\d{2}),(\d{3})/);
                if (!timeMatch) continue;
                
                var hrs = parseInt(timeMatch[1], 10);
                var mins = parseInt(timeMatch[2], 10);
                var secs = parseInt(timeMatch[3], 10);
                var ms = parseInt(timeMatch[4], 10);
                var timestamp_ms = ((hrs * 3600) + (mins * 60) + secs) * 1000 + ms;
                
                var dataLine = blockLines.slice(2).join(" ");
                var latMatch = dataLine.match(/(?:latitude|lat)\s*[:\s]\s*([-\d.]+)/i);
                var lonMatch = dataLine.match(/(?:longitude|longtitude|lon|lng)\s*[:\s]\s*([-\d.]+)/i);
                var altMatch = dataLine.match(/(?:altitude|alt|height)\s*[:\s]\s*([-\d.]+)/i);
                var headingMatch = dataLine.match(/(?:heading|bearing|compass|compass_heading)\s*[:\s]\s*([-\d.]+)/i);
                ...
                if (latMatch && lonMatch) {
                    ...
                    var gpsMatch = dataLine.match(/gps\s*[:\s(]\s*([-\d.]+)\s*,\s*([-\d.]+)\s*(?:,\s*([-\d.]+))?/i);
                    if (gpsMatch) {
                        var val1 = parseFloat(gpsMatch[1]);
                        var val2 = parseFloat(gpsMatch[2]);
                        var val3 = gpsMatch[3] ? parseFloat(gpsMatch[3]) : 0.0;
                        
                        if (Math.abs(val1) > 90.0) {
                            lon = val1;
                            lat = val2;
                        } else if (Math.abs(val2) > 90.0) {
                            lon = val2;
                            lat = val1;
                        } else {
                            lat = val1;
                ...
                if (lat !== null && lon !== null) {
                    ...
                    if (headingMatch) heading = parseFloat(headingMatch[1]);
                    
                    observations.push({
                        timestamp_ms: timestamp_ms,
                        latitude: lat,
                        longitude: lon,
                        "ascent(feet)": alt / 0.3048,
